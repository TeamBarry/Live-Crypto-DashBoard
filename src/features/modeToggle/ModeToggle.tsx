...
import React from 'react'
import { useAppDispatch, useAppSelector } from '../../hooks/redusHooks'
import { setMode, selectMode, selectLastUpdated, selectIsLoading } from '../../store/modeSlice'
import type { DashboardMode } from '../../types'
...
export function ModeToggle() {
  ...
  return (
    <div className='flex flex-col gap-2 md:gap-3 w-full'>
      ...
      {/* Buttons Container */}
      <div className="flex items-center justify-between md:justify-start gap-1 bg-gray-800 rounded-xl p-1 md:p-1.5 border border-gray-700 w-full">
        {(Object.keys(modeConfig) as DashboardMode[]).map((mode) => {
          const config = modeConfig[mode];
          const isActive = currentMode === mode;
          return (
            <button
              ...
              disabled={isLoading && mode !== currentMode}
              className={`
                flex-1 md:flex-none relative flex items-center justify-center gap-1 md:gap-2 px-2 md:px-4 py-2 rounded-lg text-xs md:text-sm font-medium
                transition-all duration-200 ease-out
                ${isActive 
                  ? config.activeColor + ' shadow-lg md:scale-105 scale-100' 
                  : config.color + ' hover:text-white hover:bg-gray-700'
                }
                ${isLoading && mode !== currentMode ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
              `}
            >
              {/* Icon */}
              <span className={`text-[10px] md:text-xs ${isActive ? 'animate-pulse' : ''}`}>
                {config.icon}
              </span>
              ...
              {/* Active indicator dot */}
              {isActive && mode === 'realtime' && (
                <span className="absolute -top-1 -right-1 w-2 md:w-2.5 h-2 md:h-2.5 bg-green-400 rounded-full animate-ping" />
              )}
            </button>
          );
        })}
      ...
      {/* Footer Details */}
      <div className="flex flex-col md:flex-row items-center md:justify-between text-[10px] md:text-xs text-gray-500 px-1 gap-1 md:gap-0 text-center md:text-left">
